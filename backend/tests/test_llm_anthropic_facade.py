"""Тесты публичных методов AnthropicFacade.

Facade берёт сервис из AnthropicFactory и проксирует его поток событий.
"""
...
def mock_factory(mock_service):
    factory = MagicMock(spec=AnthropicFactory)
...
def facade(mock_factory):
    return AnthropicFacade(mock_factory)
...
async def test_stream_landing_passes_dto_to_service(facade, mock_service):
    ...
    dto = LlmPromptTransfer(prompt="лендинг про спорт")
